...
function (hook::NatureHook)(::PostActStage, policy, env)
    for p in 1:length(env.players)
        ...
        push!(hook.food_balance, max(env.players[p].food_counts...) / min(env.players[p].food_counts...))
    end
    hook.step += 1
...
function (hook::NatureHook)(::PostEpisodeStage, policy, env)
    ...
    @info "rewards"     total_reward=sum(hook.total_rewards) log_step_increment=0
    ...
    @info "exchanges"   ex=Tuple(env.exchanges)              log_step_increment=0
    @info "food_balance" food_balance=mean(hook.food_balance)
